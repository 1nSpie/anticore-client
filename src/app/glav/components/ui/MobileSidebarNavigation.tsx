  ...
  return (
    <div>
      ...
      <button
        ...
        className="lg:hidden text-greenDefault p-2 rounded-md"
      >
        ...
        <div className="flex flex-col h-full justify-between">
          <div>
            <div className="mx-auto p-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
              ...
              <button
                onClick={() => setIsOpen(false)}
                className="text-greenDefault hover:text-greenDefaultHover transition-colors"
            ...
            <nav className="p-4 space-y-4">
              {navigationLinks.map((item, index) => (
                <Link
                  key={index}
                  href={item.link}
                  className="block text-lg font-medium text-gray-800 dark:text-gray-200 hover:text-greenDefault transition-transform transform hover:translate-x-1"
                  onClick={() => setIsOpen(false)}
                >
                  {item.label}
                </Link>
